  ...
  public void syncContact(Office365Account office365Account)
    ...
    List<Partner> partnerList =
        partnerQuery
            .add(queryStr)
            .bind("start", start)
            .bind("user", office365Account.getOwnerUser())
            .build()
            .fetch();
    ...
    if (office365Account.getLastCalendarSyncOn() != null) {
      calendarQuery =
          calendarQuery
              .add(lastSyncQuery)
              .add(
                  "((self.isOffice365Object != true AND self.user = :user) "
      ...
      calendarQuery = calendarQuery.add(query).add("self.user = :user");
      eventQuery = eventQuery.add(query).add("self.user = :user");
    }
